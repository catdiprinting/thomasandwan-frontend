...
export default function FAQSection({
  ...
  subtext = "Navigating medical malpractice claims can be confusing. Here are answers to some of the most common questions our clients ask.",
}: FAQSectionProps) {
  return (
      ...
      <div className="container mx-auto px-4 md:px-6">
        <div className="grid lg:grid-cols-12 gap-16">
          <div className="lg:col-span-5">
            <span className="text-secondary font-bold tracking-widest uppercase text-sm mb-4 block" data-testid="text-faq-label">
              {label}
            </span>
            <h2 className="text-4xl md:text-5xl font-serif text-primary mb-6" data-testid="text-faq-heading">
              {heading}
            </h2>
            <p className="text-lg text-muted-foreground font-light mb-8" data-testid="text-faq-subtext">
              {subtext}
            </p>
            <div className="p-8 bg-primary text-white relative overflow-hidden">
              <div className="relative z-10">
                ...
                <button className="bg-secondary text-primary hover:bg-white transition-colors px-6 py-3 font-bold uppercase tracking-wide w-full" data-testid="button-faq-contact">
                  Contact Us Now
                </button>
              </div>
            ...
            <Accordion type="single" collapsible className="w-full">
              {faqs.map((faq, idx) => (
                <AccordionItem key={idx} value={`item-${idx}`} className="border-b border-gray-100">
                  <AccordionTrigger className="text-xl font-serif text-primary hover:text-secondary transition-colors py-6 text-left" data-testid={`button-faq-${idx}`}>
                    {faq.question}
                  </AccordionTrigger>
